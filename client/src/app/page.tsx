    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-24">
      ...
      {result && (
        <div className="mt-8 p-4 bg-gray-100 rounded-md w-full max-w-2xl">
          ...
          <p>
            <strong>Content Type:</strong> {result.content}
          </p>
          <p>
            <strong>Predicted Class:</strong>{" "}
            {result.predicted_class === 1 ? "Real" : "Fake"}
          </p>
          <p>
            <strong>Probabilities:</strong> Real:{" "}
            {(result.probabilities[1] * 100).toFixed(2)}%, Fake:{" "}
            {(result.probabilities[0] * 100).toFixed(2)}%
          </p>
          <p>
            <strong>Positive Features:</strong>{" "}
            {result?.positive_features?.length > 0
              ? result.positive_features.join(", ")
